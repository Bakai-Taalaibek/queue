...
const HomePage = () => {
  ...
  const getProfileInfoLoading = useRegistrar(
    (state) => state.getProfileInfoLoading
  );
  ...
  const columns = [
   ...
   },
   {
     ...
     render: (value) => (
       <p className={styles.columnData}>{returnUnderstandableDate(value)}</p>
     ),
   },
   {
         ...
         <div className={styles.columnServiceBlock}>
           {talon?.status === "completed" ? (
             <>
               ...
               {calculateTimeDifference(talon?.service_start, talon?.service_end) > 3 && talon?.service_end ? (
             ...
             <>
               ...
               <div className={styles.childDiv}>
                 <Popover content={tableContent(talon)} trigger="hover" placement="leftTop" id="tablePopover">
  ...
  const filteredTalons = talons.filter((talon) =>
  talon.token.includes(searchValue)
);
